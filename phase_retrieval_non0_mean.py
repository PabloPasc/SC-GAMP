...
#STATE EVOLUTION - version used to find fixed points
#@jit(nopython=True)
def se_iid_pr_exact_tau0(delta, a, prob, it, tau_0=None):
...
#@jit(nopython=True)
def pr_gamp_iid_non0_mean(A, A_T, y, t, a, prob, x_0):
    m, n = len(A), len(A[0])
    delta = m/n
    # Initialise
    p = np.zeros(m)
    ...
    print(tau_q)
    # Calculate q
    q = g_in_q + tau_q*np.dot(A_T, g_out_pr(p, y, tau_p))
    ...
    for it in range(t):
        ...
        # Calculate p
        p = np.dot(A, g_in_q)- tau_p*g_out_pr(p, y, tau_p_prev)
        #print("p", p )
        """
        expec = quad(integral_2, -30,30, args=(tau_p, exp_z2))[0]
        tau_p_inv = 1/tau_p
...
err, norm, x_hat = pr_gamp_iid_non0_mean(A_iid, A_iid_T, y, 300, a, prob, x_0)"""
...
plt.xlabel(r"Column block index $c$")
plt.ylabel(r"Mean Squared Error")
plt.plot(mmse_vec_array[0], label="t={}".format(1), color='red', alpha=1)
plt.legend(bbox_to_anchor=(1.15, 0.85), framealpha=1, fontsize=12)
plt.grid(alpha=0.4, linestyle="--")
plt.rc('axes', labelsize=12) #fontsize of the x and y labels
plt.rc('xtick', labelsize=12) #fontsize of the x tick labels
plt.rc('ytick', labelsize=12) #fontsize of the y tick labels
plt.ylim(-0.0495, 1.0395)
plt.tight_layout()
...
plt.xlabel(r"Column block index $c$")
plt.ylabel(r"Mean Squared Error")
plt.plot(mmse_vec_array[0], label="t={}".format(1), color='red', alpha=0.1)
plt.plot(mmse_vec_array[9], label="t={}".format(10), color='red', alpha=1)
plt.legend(bbox_to_anchor=(1.15, 0.85), framealpha=1, fontsize=12)
plt.grid(alpha=0.4, linestyle="--")
plt.rc('axes', labelsize=12) #fontsize of the x and y labels
plt.rc('xtick', labelsize=12) #fontsize of the x tick labels
...
plt.ylabel(r"Mean Squared Error")
plt.plot(mmse_vec_array[0], label="t={}".format(1), color='red', alpha=0.1)
plt.plot(mmse_vec_array[9], label="t={}".format(10), color='red', alpha=0.1)
plt.plot(mmse_vec_array[19], label="t={}".format(20), color='red', alpha=1)
plt.legend(bbox_to_anchor=(1.15, 0.85), framealpha=1, fontsize=12)
plt.grid(alpha=0.4, linestyle="--")
plt.rc('axes', labelsize=12) #fontsize of the x and y labels
plt.rc('xtick', labelsize=12) #fontsize of the x tick labels
...
plt.plot(mmse_vec_array[0], label="t={}".format(1), color='red', alpha=0.1)
plt.plot(mmse_vec_array[9], label="t={}".format(10), color='red', alpha=0.1)
plt.plot(mmse_vec_array[19], label="t={}".format(20), color='red', alpha=0.1)
plt.plot(mmse_vec_array[29], label="t={}".format(30), color='red', alpha=1)
plt.legend(bbox_to_anchor=(1.15, 0.85), framealpha=1, fontsize=12)
plt.grid(alpha=0.4, linestyle="--")
plt.rc('axes', labelsize=12) #fontsize of the x and y labels
plt.rc('xtick', labelsize=12) #fontsize of the x tick labels
...
plt.plot(mmse_vec_array[9], label="t={}".format(10), color='red', alpha=0.1)
plt.plot(mmse_vec_array[19], label="t={}".format(20), color='red', alpha=0.1)
plt.plot(mmse_vec_array[29], label="t={}".format(30), color='red', alpha=0.1)
plt.plot(mmse_vec_array[38], label="t={}".format(39), color='red', alpha=1)
plt.legend(bbox_to_anchor=(1.15, 0.85), framealpha=1, fontsize=12)
plt.grid(alpha=0.4, linestyle="--")
plt.rc('axes', labelsize=12) #fontsize of the x and y labels
plt.rc('xtick', labelsize=12) #fontsize of the x tick labels
plt.rc('ytick', labelsize=12) #fontsize of the y tick labels
plt.ylim(-0.0495, 1.0395)
tikzplotlib.save('wave_prop.tex')"""


#@jit(nopython=True)
def pr_gamp_sc_non0_mean(W, A, A_T, y, x_0, a, prob, t):
    m, n = len(A), len(A[0])
    R, C = len(W), len(W[0])
    M = int(m/R)#
    N = int(n/C)
    ...
    for c in range(C):
        tau_q_A_gout[N*c: (N*c + N)] = tau_q[c]*np.dot(A_T, g_out_p)[N*c:(N*c+N)]
...
m = int(delta*n)
x_0 = produce_data.create_x_0_non0_mean(n, a, prob)
A = produce_data.create_A_iid(m, n)#produce_data.create_A_sc(W, N, delta_hat)
A_T = np.transpose(A)
y = (np.dot(A, x_0))**2

error_norm_array, nc_array, x_hat = pr_gamp_iid_non0_mean(A, A_T, y, 300, a, prob, x_0)
plt.figure()
