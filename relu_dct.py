...
#print(se_iid_cs_exact(1.5, 1, 1/np.sqrt(1), 100))
"""
nc_pred_array = []
mse_pred_array = []
...
def relu_gamp_iid_dct(m, n, y, t, eps, a, x_0, rand_seed, Ax=None, Ay=None, exp_x=0):
...
while it < run_no:
    ...
    Ax, Ay = produce_data_dct.sub_dct_iid(m, n)

    x_0 = produce_data.create_x_0(eps, n, "Discrete")*(1/np.sqrt(eps))
    y = np.maximum(Ax(x_0),0)
    
    
    #error_norm_array, x_hat= cs_gamp_iid_new(A_iid, y, 300, eps, 1/np.sqrt(eps), x_0, exp_x=0)
    error_norm_array, norm_correl_array, x_hat = relu_gamp_iid_dct(m, n, y, 300, eps, 1/np.sqrt(eps), x_0, rand_seed, Ax, Ay)
    it += 1
"""
...
def relu_gamp_sc_dct(W, m, n, y, x_0, eps, a, t, rand_seed, Ab=None, Az=None,exp_x=0):
    ...
    for c in range(C):
        tau_q_A_gout[N*c: (N*c + N)] = tau_q[c]*Az(g_out_array)[N*c:(N*c+N)]
...
while run_no < 10:
    ...
    x_0 = produce_data.create_x_0(eps, n, "Discrete")*(1/np.sqrt(eps))
    print(x_0)
    y = np.maximum(Ab(x_0),0) #+ np.random.normal(0, 0.0001, size=int(delta*n))
    
    
    error_norm_array, norm_correl_array, x_hat = relu_gamp_sc_dct(W, m, n, y, x_0, eps, 1/np.sqrt(eps), t, rand_seed, Ab, Az)
