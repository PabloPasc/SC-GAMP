...
for x in dom:
    print(x)
    g_x.append(1000 - (delta/x)*(1-(delta/x)*expec_var_relu(1, x, delta)))
plt.figure()
plt.plot(dom, g_x)
plt.xlabel(r'$x$')
plt.ylabel(r'$g(x)$')
...
#print(scalar_pot_relu(1e-3, 0.5, 0.5, 1/np.sqrt(eps)))

mmse_bound, iid_bound, _ = relu_bound_disc(delta, np.linspace(1e-3,0.995,num=51),eps,1/np.sqrt(eps))

print("Delta: ", delta)
print("MMSE bound", mmse_bound)
print("iid bound", iid_bound)
...
plt.plot(delta_domain_ext, mse_bound_array1, label="MMSE bound")
plt.plot(delta_domain_ext, iid_bound_array1, label="iid bound")"""
